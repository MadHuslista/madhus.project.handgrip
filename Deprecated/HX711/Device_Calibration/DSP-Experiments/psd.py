...
def psd(x, fs=1.0, window='hanning', nperseg=None, noverlap=None, nfft=None,
    ...
    if show:
        _plot(x, fs, f, P, mpf, fmax, fpcntile, scales, xlim, units, ax, pctil)

    return fpcntile, mpf, fmax, Ptotal, f, P


def _plot(x, fs, f, P, mpf, fmax, fpcntile, scales, xlim, units, ax, pctil):
    ...
    try:
        ...
        ax.plot([mpf, mpf], ylim, 'r--', lw=1.5,
                 label='Fmean = %.2f' % mpf)
        ax.plot([fpcntile[pctil], fpcntile[pctil]], ylim, 'r:', lw=2,
                 label='F%s%%   = %.2f' % (pctil,fpcntile[pctil]))
