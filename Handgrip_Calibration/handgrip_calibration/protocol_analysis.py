...
def _window(df: pd.DataFrame, start: float, end: float) -> pd.DataFrame:
    if df.empty or "timestamp_lsl" not in df.columns or not np.isfinite(start) or not np.isfinite(end):
        return pd.DataFrame()
...
def _stream_row(name: str, df: pd.DataFrame, preferred: list[str]) -> dict[str, Any]:
    ...
    row = {
        ...
        "duration_s": duration,
        "sample_rate_hz": float((len(df) - 1) / duration) if duration > 0 and len(df) > 1 else np.nan,
    ...
    if len(values):
        row.update({
            ...
            "std": float(np.nanstd(values, ddof=1)) if np.count_nonzero(np.isfinite(values)) > 1 else 0.0,
...
def event_count_table(session_dir: str | Path) -> pd.DataFrame:
    ...
    return frame.groupby("event", dropna=False).size().reset_index(name="count").sort_values("event")
...
def hold_quality_summary(dataset: pd.DataFrame) -> dict[str, Any]:
    ...
    out: dict[str, Any] = {
        ...
        "quality_pass_holds": int(dataset.get("accepted_by_quality", pd.Series(dtype=bool)).fillna(False).sum()) if "accepted_by_quality" in dataset else None,
    }
    for col in ["reference_force_std_N", "reference_slope_N_s", "reference_sample_rate_hz", "target_sample_rate_hz", "target_seq_gap_count"]:
...
def hysteresis_summary(dataset: pd.DataFrame) -> pd.DataFrame:
    ...
    for force, group in dataset.groupby("target_force_nominal_N"):
        ...
        if "target_raw_median" in group.columns:
            row["target_raw_delta_desc_minus_asc"] = float(desc["target_raw_median"].median() - asc["target_raw_median"].median())
        if "reference_force_median_N" in group.columns:
            row["reference_force_delta_desc_minus_asc_N"] = float(desc["reference_force_median_N"].median() - asc["reference_force_median_N"].median())
        rows.append(row)
...
def creep_zero_return_summary(session_dir: str | Path) -> pd.DataFrame:
    ...
    if ref_col is None:
        return pd.DataFrame()
    start_events = [(idx, e) for idx, e in enumerate(events) if e.get("event") in {"creep_start", "zero_return_start"}]
    rows: list[dict[str, Any]] = []
    for idx, start in start_events:
        ...
        slope = float(np.polyfit(rel_t, y, deg=1)[0]) if len(w) >= 2 and np.nanmax(rel_t) > 0 else np.nan
        ...
        rows.append({
            ...
            "reference_start_mean_N": float(first[ref_col].mean()) if not first.empty else np.nan,
            "reference_end_mean_N": float(last[ref_col].mean()) if not last.empty else np.nan,
            "delta_end_minus_start_N": float(last[ref_col].mean() - first[ref_col].mean()) if not first.empty and not last.empty else np.nan,
            "slope_N_per_s": slope,
        })
    return pd.DataFrame(rows)
...
def dynamic_summary(session_dir: str | Path) -> pd.DataFrame:
    ...
    if not events:
        return pd.DataFrame()
    rows: list[dict[str, Any]] = []
    for start_name, end_name, label in [("ramp_start", "ramp_end", "ramp"), ("squeeze_start", "squeeze_end", "squeeze")]:
