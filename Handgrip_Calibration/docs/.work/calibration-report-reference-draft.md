...
- **Companion documents**: `docs/.work/report-concept-inventory.md` and `docs/.work/calibration-report-reference-source-inventory.md`.
- **Purpose**: This document is a draft intended to facilitate the generation of the calibration report reference by providing a starting point from which a complete reference can be developed. Evaluate usefulness BEFORE investing time.
- **Version**: **[DRAFT]**. 
---

> **WARNING:** 
> This document is a draft and may contain inaccuracies or incomplete sections. 
> It is intended to facilitate the generation of the calibration report reference by providing a starting point from which a complete reference can be developed. 
> **Evaluate if this draft is pointing in the right direction and contains the necessary sections before investing time in completing it**. 
> If the structure and content are **NOT appropriate**, ignore the draft, do not use it, and start de novo. 
> If the structure and content are appropriate, verify accuracy, move the draft to `Handgrip_Calibration/docs/`, continue its development, and finalize the reference.
...
This diagnostic affine model approximates orthogonal distance regression / Deming regression. ODR is used when there is uncertainty in both input and output variables; SciPy describes ODRPACK as supporting explicit or implicit ODR and ordinary least squares [R07].
