...
class MeasurementFrame:
    ...
    # Calibration session identifier propagated to logs and IPC so that
    # downstream components can bind reference samples to a recorder session.
    session_id: str = ''
    # Static board/session metadata snapshot. This is intentionally copied into
    # every IPC frame because ZeroMQ PUB/SUB subscribers may start late.
...
def get_session_id(cfg: DictConfig) -> str:
    """Return the optional calibration session identifier configured for this GUI.

    The authoritative calibration session is created by Handgrip_Calibration. This
    value is a pass-through hook: set it only when you want logs/IPC generated by
    RS485_GUI to be trivially grouped with a specific recording session.
    """
    try:
        return str(getattr(cfg.session, 'session_id', '') or '')
    except Exception:
        return ''


def build_board_profile_snapshot(cfg: DictConfig) -> Dict[str, Any]:
    """Build a compact board/config snapshot for calibration manifests.

    This snapshot captures the acquisition-board settings that matter for force
    calibration: serial format, Modbus/Active-Send mode, rate target, parser
    profile, and the recommended PM58/reference-chain assumptions.
    """
    return {
        ...
        'recommended_unit': 'N',
        'serial': {
            'port': str(getattr(cfg.serial, 'default_port', '') or ''),
            'baudrate': int(getattr(cfg.serial, 'default_baudrate', 460800)),
            'parity': str(getattr(cfg.serial, 'default_parity', 'N')),
            'stopbits': int(getattr(cfg.serial, 'default_stopbits', 1)),
            'bytesize': int(getattr(cfg.serial, 'bytesize', 8)),
        },
        'device': {
            'mode': str(getattr(cfg.device, 'mode', 'active_send')),
            'slave_address': int(getattr(cfg.device, 'slave_address', 1)),
            'active_send_frequency_code': int(getattr(cfg.device, 'active_send_frequency_code', 8)),
            'poll_interval_s': float(getattr(cfg.device, 'poll_interval_s', 0.001)),
        },
        'active_send': {
            'timestamp_policy': str(getattr(cfg.active_send, 'timestamp_policy', 'batch_end_anchored')),
            'parser_profile': str(getattr(cfg.active_send, 'default_parser_profile', 'modbus_rtu_response_11regs')),
            'frame_register_count': int(getattr(cfg.active_send, 'frame_register_count', 11)),
            'delivery_window_s': float(getattr(cfg.active_send, 'delivery_window_s', 0.010)),
            'max_frames_per_delivery': int(getattr(cfg.active_send, 'max_frames_per_delivery', 16)),
        },
        'ipc': {
            'topic': str(getattr(cfg.ipc, 'topic', 'rs485.measurement.v1')),
            'signal_key': str(getattr(cfg.ipc, 'signal_key', 'net_value')),
            'publish_after_max_rate_filter': bool(getattr(cfg.ipc, 'publish_after_max_rate_filter', False)),
        },
    }
...
class MeasurementFramePublisher:
    ...
    def __init__(self, cfg: DictConfig):
        ...
        self.event_topic = str(getattr(cfg.ipc, 'event_topic', 'rs485.event.v1'))
        self.signal_key = str(getattr(cfg.ipc, 'signal_key', 'net_value'))
        self.session_id = get_session_id(cfg)
        self.board_profile = build_board_profile_snapshot(cfg)
    ...
    def start(self) -> None:
        ...
        self._socket = socket
        LOGGER.info('RS485 IPC publisher bound to %s topic=%s event_topic=%s signal_key=%s', self.bind, self.topic, self.event_topic, self.signal_key)
    ...
    def publish_event(self, event: str, **payload: Any) -> None:
        """Publish a structured operational event on a separate IPC topic.

        The LSL_Bridge subscribes only to measurement frames, so events use their
        own topic to avoid contaminating the high-rate reference stream. These
        messages are useful for future dashboards/session manifests and harmless
        if no subscriber is present.
        """
        if not self.enabled:
            return
        with self._lock:
            ...
            try:
                ...
                # Events are diagnostic and must never perturb acquisition.
                pass
    ...
    def _build_record(self, frame: MeasurementFrame) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            # Canonical v2 fields consumed by the upgraded LSL_Bridge.
            ...
            # Retained as same-schema aliases inside the v2 message for human grep/debug.
...
class SignalFileLogger:
    ...
    def open(self) -> None:
        ...
        if self.write_mode == 'overwrite' or not gui_file_preexisting:
            self._gui_writer.writerow(['host_ts_epoch_s', 'host_ts_iso', 'session_id', 'mode', 'reference_force_N', 'reference_clock_s', 'reference_status', 'plot_signal_key', 'plot_value'])
...
def decode_modbus_measurement(
    ...
    interpreted = {
        ...
        'raw_value': gross_raw,
        # Canonical calibration v2 aliases. The selected reference force is
        # configured by ipc.signal_key, but net_value is the recommended default.
        'reference_clock_s': float(rs485_clock),
...
class ModbusBoardTransport(BoardTransport):
    ...
    def read_once(self) -> MeasurementFrame:
        ...
        frame.session_id = get_session_id(self.app_state.cfg)
        frame.board_profile = build_board_profile_snapshot(self.app_state.cfg)
        return frame
...
class ActiveSendBoardTransport(BoardTransport):
    ...
    def read_frames(self) -> List[MeasurementFrame]:
        ...
        if profile == 'modbus_rtu_response_11regs':
            ...
            for idx, frame_bytes in enumerate(frame_bytes_batch):
                ...
                decoded_frame.interpreted['reference_clock_s'] = decoded_frame.interpreted.get('rs485_clock')
                decoded_frame.interpreted['reference_force_N'] = decoded_frame.interpreted.get('net_value', decoded_frame.interpreted.get('raw_value'))
                decoded_frame.interpreted['reference_status'] = decoded_frame.interpreted.get('status_word', 0)
                decoded_frame.session_id = get_session_id(self.app_state.cfg)
                decoded_frame.board_profile = build_board_profile_snapshot(self.app_state.cfg)
    ...
    def read_once(self) -> MeasurementFrame:
        """Read one active-send frame through the canonical binary Modbus profile.

        Non-Modbus ASCII/hex debug profiles were removed from the calibration
        architecture because they do not provide the documented 11-register
        force/status payload required for calibration QA.
        """
        if self.app_state.parse_profile != 'modbus_rtu_response_11regs':
            raise RuntimeError(
                'Calibration schema v2 supports only active_send.default_parser_profile=modbus_rtu_response_11regs'
            )
        frames = self.read_frames()
        if not frames:
            raise TimeoutError('No active-send frames decoded')
        return frames[-1]
...
def disconnect_state(app_state: AppState) -> None:
    if app_state.ipc_publisher is not None:
        app_state.ipc_publisher.publish_event('serial_disconnect_requested', port=app_state.serial_cfg.port, mode=app_state.mode)
...
def connect_state(app_state: AppState) -> None:
    ...
    try:
        ...
        if bool(getattr(app_state.cfg.ipc, 'enabled', False)) and bool(getattr(app_state.cfg.ipc, 'start_on_connect', True)):
            ...
            if app_state.ipc_publisher is not None:
                app_state.ipc_publisher.publish_event('serial_connected', port=app_state.serial_cfg.port, mode=app_state.mode)
    except Exception:
        if app_state.ipc_publisher is not None and bool(getattr(app_state.cfg.ipc, 'stop_on_disconnect', True)):
            app_state.ipc_publisher.stop()
        if app_state.transport is not None:
