...
def build_signal_metadata_text(
    ...
    selected_values = [
        v for v in (extract_signal_value(f, signal_key) for f in frame_history) if v is not None
    ]
    if selected_values:
        value_range_line = (
...
#  @return Retrieved value for this request.
def get_target_sampling_rate_hz(cfg: DictConfig, mode: str) -> float | None:
    ...
    if mode == "active_send":
        return float(
            ACTIVE_SEND_FREQ_CODE_TO_VALUE.get(int(cfg.device.active_send_frequency_code), 0) or 0
        )
