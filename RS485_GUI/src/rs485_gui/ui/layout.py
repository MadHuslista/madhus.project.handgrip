    ...
    *connect_fn* and *disconnect_fn* are callables from ``app.py`` that own
    ...
    with ui.card().classes("w-full"):
        ...
        with ui.grid(columns=4).classes("w-full gap-4"):
            port_select = ui.select(
                options={
                    p.device: f"{p.device} | {p.description} | {p.hwid}" for p in available_ports
                },
                value=(
        ...
        board_cfg_preview = (
            ui.textarea(value="", label="Board-side values to mirror")
            .props("readonly")
            .classes("w-full font-mono")
        )
        board_cfg_preview.style(
            "height: 180px; overflow-y: auto; overflow-x: auto; white-space: pre;"
        )
    ...
    with ui.row().classes("w-full gap-4 items-start mt-4 no-wrap"):
        with ui.card().classes("w-1/2"):
            ...
            raw_log_area = (
                ui.textarea(value="", label="Raw log").props("readonly").classes("w-full font-mono")
            )
            raw_log_area.style(
        ...
        with ui.card().classes("w-1/2"):
            ...
            interpreted_log_area = (
                ui.textarea(value="", label="Interpreted log")
                .props("readonly")
                .classes("w-full font-mono")
            )
    ...
    with ui.card().classes("w-full mt-4"):
        ...
        event_log_area = (
            ui.textarea(value="", label="Events").props("readonly").classes("w-full font-mono")
        )
