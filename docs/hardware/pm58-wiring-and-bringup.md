...
1. Keep board power disconnected while wiring.
2. Wire PM58 bridge lines to acquisition-board sensor terminals: `red -> E+ / 5`, `black -> E- / 6`, `white -> S- / 7`, `green -> S+ / 8`.
3. Treat the fifth PM58 conductor as shield/drain until continuity testing proves otherwise; do not land it on terminals 5-8 during first bring-up.
4. Wire the AC-powered board as `19 = L`, `20 = N` only if the unit label confirms AC100-240V.
5. Wire RS485 as `1 = A+`, `2 = B-` to the USB-RS485 adapter.
6. Power on, confirm display startup, confirm force response, then validate PC communication.
